import tensorflow as tf
import numpy as np
from matplotlib import pyplot as plt
import os, sys
import aes
import random
from tqdm import tqdm

sys.path.append('../correlation')
import correlation_tools as ct

DIR = '../data/our-data/for_training/100k_d10_k2_100avg'

NUMBER = 1000
average = 100

byteOfInterest = 6
epoch = 40

AES_Sbox = np.array([
	
])

Inv_Sbox = []

def main():
	traces, pt, key = load_data(DIR)
	key = key.reshape(16)
	model = tf.keras.models.load_model('./models/byte' + str(byteOfInterest) + '/attack_model_byte' + str(byteOfInterest) + '-' + str(epoch) + '.h5')
	ranks_array = []
	sum_prob_array = []

	for i in tqdm(range(average), ncols=60):
		select = random.sample(range(len(traces)), NUMBER)
		
		keyOfInterest = key[byteOfInterest]
		ptOfInterest = pt[select, byteOfInterest]
		
		predictions = get_prediction(model, traces[select]) # Get Sbox output predictions
		probabilities = get_probabilities(predictions, ptOfInterest) # Get Key proababilities from Sbox output prediciotns
		
		sum_prob = np.sum(np.log(probabilities), axis=0)
		sum_prob_array.append(sum_prob)		

		ranks = rank_cal(probabilities, keyOfInterest, NUMBER)
		ranks_array.append(ranks)
	print(np.argmax(np.sum(sum_prob_array, axis=0)))
	print(keyOfInterest)
	ranks_array = np.array(ranks_array)
	#print('Real KEY: ' + str(keyOfInterest))
	#print('Guessed KEY: ' + str(np.argmax(ranks[1])/(average*NUMBER)))

	for i in range(ranks_array.shape[1]):

		if np.count_nonzero(ranks_array[:, i]) < int(average / 2):
		
			print(i)
			break
	
	average_ranks = np.sum(ranks_array, axis=0) / average
			
	np.save('average_ranks.npy',average_ranks)
	plt.plot(average_ranks)
	plt.show()


def load_data(folder, norm=True):

	if norm:
		raw = np.memmap(folder + '/traces.npy', dtype='float32', mode='r', shape=(100000,4500))[:100000, 1139:1249]
		traces = np.zeros((100000, 110))
		for trace in range(len(raw)):
			traces[trace] = ct.normMaxMin(raw[trace])
	else:
		traces = np.memmap(folder + '/traces.npy', dtype='float32', mode='r', shape=(100000,4500))[:100000, 1139:1249]
		traces = traces.reshape(110, len(traces)) # Transpose
	
	pt = np.load(folder + '/pt.npy')
	key = np.load(folder + '/key.npy')

	return traces, pt, key

def get_prediction(model, Traces):
	# Get the input layer shape
	input_layer_shape = model.get_layer(index=0).input_shape

	# Sanity check
	if input_layer_shape[1] != len(Traces[0]):
		print("Error: model input shape %d instead of %d is not expected ..." % (
			input_layer_shape[1], len(Traces[0])))
		sys.exit(-1)

	# Adapt the data shape according our model input
	if len(input_layer_shape) == 2:
		# This is a MLP
		input_data = Traces
	elif len(input_layer_shape) == 3:
		# This is a CNN: reshape the data
		input_data = Traces
		input_data = input_data.reshape((input_data.shape[0], input_data.shape[1], 1))
	else:
		print("Error: model input shape length %d is not expected ..." % len(input_layer_shape))
		sys.exit(-1)

	# Predict our probabilities
	predictions = model.predict(input_data)

	return predictions

def get_probabilities(predictions, pt):
	probabilities_array = []

	for i in range(NUMBER):
		probabilities = np.zeros(256)
		for j in range(256):
			value = Inv_Sbox[pt[i] ^ j]				
			probabilities[j] = predictions[i][value]

		probabilities_array.append(probabilities)
		

	probabilities_array = np.array(probabilities_array)

	for i in range(len(probabilities_array)):
		if np.count_nonzero(probabilities_array[i]) != 256:
			none_zero_predictions = [a for a in probabilities_array[i] if a != 0]

			min_v = min(none_zero_predictions)

			#print(min_v**2)
			probabilities_array[i] = probabilities_array[i] + min_v**2

	return probabilities_array

def rank_cal(selected_probabilities, key_interest, NUMBER):
	rank = []

	total_pro = np.zeros(256)

	for i in range(NUMBER):
		total_pro = total_pro + np.log(selected_probabilities[i])
		# find the rank of real key in the total probabilities

		sorted_proba = np.array(list(map(lambda a: total_pro[a], total_pro.argsort()[::-1])))
		key_rank = np.where(sorted_proba == total_pro[key_interest])[0][0]

		rank.append(key_rank)

	rank = np.array(rank)

	return rank

if __name__ == "__main__":
	main()