...
def main():
    ...
    for i in range(1,numberOfKeys+1): # 1, 2, 3, 4, 5
        ...
        plaintext = np.load(dir + '/pt.npy')[:1000]
        ...
        for text in plaintext:
            ...
            printProgressBar(count, total, prefix = 'Key ' + str(i) + '/' + str(numberOfKeys) + ' Progress:', suffix = 'Complete, ETA: '+str(round((eta/60)))+' min', length = 50)
            ...
            raw = np.fromfile(outputfile, dtype='float32')[-100000:] # Cut the crap
            if len(ct.getTracesFromArray(raw, template)) <= 8:
                print(str(count-1) + ' ' + str(len(ct.getTracesFromArray(raw, template))))
